...
DshotAnalyzer::DshotAnalyzer()
    : Analyzer(), mSettings(new DshotAnalyzerSettings()),
      mSimulationInitilized(false) {
  SetAnalyzerSettings(mSettings.get());
}

DshotAnalyzer::~DshotAnalyzer() { KillThread(); }
...
void DshotAnalyzer::WorkerThread() {
  ...
  for (;;) {
    ...
    for (i = sizeof(data) * 8 - 1; i >= 0; i--) {
      ...
      bool error = i > 0 && proportionOfBit(mSerial->GetSampleOfNextEdge() -
                                            falling_sample) > 1.5;
      ...
      mResults->AddMarker(rising_sample + width / 2, marker,
    ...
    uint8_t crc =
        ((chan >> 4) & 0xf) ^ ((chan >> 8) & 0xf) ^ ((chan >> 12) & 0xf);
    ...
    if (!crcok)
      mResults->AddMarker(mSerial->GetSampleNumber(), AnalyzerResults::ErrorX,
                          mSettings->mInputChannel);

    // we have a byte to save.
    Frame frame;
    frame.mData1 = chan;
    frame.mFlags = (crcok ? 0 : DISPLAY_AS_ERROR_FLAG) |
                   ((data & 0x10) > 0); // error flag | telem request
...
bool DshotAnalyzer::NeedsRerun() { return false; }

U32 DshotAnalyzer::GenerateSimulationData(
...
const char *DshotAnalyzer::GetAnalyzerName() const { return "Dshot"; }

const char *GetAnalyzerName() { return "Dshot"; }

Analyzer *CreateAnalyzer() { return new DshotAnalyzer(); }
